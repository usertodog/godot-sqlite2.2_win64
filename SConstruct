...
if platform == 'android':
	...
	toolchain = "c:/msys64/home/t/android_ndk/toolchains/llvm/prebuilt/" + host_platform
...
if platform == 'android': # Fix non-existant outputs
	if target == 'debug':
		Install('demo/lib/gdsqlite/debug', source=File('bin/' + env['SHLIBPREFIX'] + output + env['SHLIBSUFFIX']))
	else:
		Install('demo/lib/gdsqlite/release', source=File('bin/' + env['SHLIBPREFIX'] + output + env['SHLIBSUFFIX']))
else:
	if target == 'debug':
		Install('demo/lib/gdsqlite/debug', source=library);
	else:
		Install('demo/lib/gdsqlite/release', source=library);
